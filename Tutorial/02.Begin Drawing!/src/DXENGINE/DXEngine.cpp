...
bool DXENGINE::InitScene()
{
	...
	hr = D3DX11CompileFromFile( this->VertexShader, 0, 0, "VS", "vs_4_0", 0, 0, 0, &(this->m_VS_Buffer), 0, 0 );
	if ( FAILED( hr ) )
	{
	...
	hr = D3DX11CompileFromFile( this->PixelShader, 0, 0, "PS", "ps_4_0", 0, 0, 0, &(this->m_PS_Buffer), 0, 0 );
	if ( FAILED( hr ) )
	{
	...
	hr = this->m_D3D11Device->CreateBuffer( &vertexBufferDesc, &vertexBufferData, &(this->m_TriangleVertBuffer ) );
	if ( FAILED( hr ) )
	{
		LOG_ERROR(" Create Triangle Vertex Buffer - Failed \n ");
		return false;
	}
	else
	{
		LOG_INFO(" Create Triangle Vertex Buffer - Succssed \n ");
	}
	...
	UINT offset = 0;
	this->m_D3D11DevCon->IASetVertexBuffers( 0, 1, &(this->m_TriangleVertBuffer), &stride, &offset );
